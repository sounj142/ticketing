...
it('return 403 if user try to cancel order belong to other user', async () => {
  ...
  const order = await OrderModel.findById(createdOrder.id).populate('ticket');
  expect(order!.userId).toEqual(createdOrder.userId);
  expect(order!.ticket._id).toEqual(createdOrder.ticket.id);
});
...
it('cancel an order with valid inputs', async () => {
  ...
  expect(orders[0].ticket._id).toEqual(order.ticket.id);
